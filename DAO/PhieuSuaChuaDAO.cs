...
namespace DAO
{
    public class PhieuSuaChuaDAO
    {
        ...
        {
            string query = "Select * from VATTUPHUTUNG where MaVTPT = " + maVTPT;
            DataTable result = DataProvider.Instance.ExecuteQuery(query);
            int DonGia = int.Parse(result.Rows[0][3].ToString());
            //DataTable dt = DataProvider.Instance.ExecuteQuery("Select GiaTri from THAMSO where MaThamSo='TS1'");
            //int GiaTriPhanTramDonGiaBan = int.Parse(dt.Rows[0][0].ToString());
            // int DonGiaBanTheoPhanTram = (GiaTriPhanTramDonGiaBan * DonGia) / 100;
            return DonGia;
        }
        ...
        public string LayNoiDungTienCong(string maTienCong)//Lấy nội dung của tiền công theo mã
        {
            string query = "Select * from TIENCONG where MaTC = " + maTienCong;
            DataTable result = DataProvider.Instance.ExecuteQuery(query);
        ...
        public void LuuPhieuSuaChua(string BienSo, int TienCong, int TienPhuTung, int TongTien, DataTable TC, DataTable VTPT)//Lưu dữ liệu được nhập vào 2 bảng PHIEUSUACHUA và CHITIETPHIEUSUACHUA
        {
            ...
            if (slvtpt >= sltc)
            {
                ...
                {
                    query = "insert into CT_PHIEUSUACHUA values (" + ma + " , " + TC.Rows[i][3].ToString() + " , " + VTPT.Rows[i][5].ToString() + " , " + VTPT.Rows[i][2] + ")";
                    re = DataProvider.Instance.ExecuteNonQuery(query);
                }
                for (int i = sltc; i < slvtpt; i++)
                {
                    query = "insert into CT_PHIEUSUACHUA ( MaPhieuSuaChua, MaVTPT, SoLuongVTPT) values (" + ma + " , " + VTPT.Rows[i][5].ToString() + " , " + VTPT.Rows[i][2] + ")";
                    re = DataProvider.Instance.ExecuteNonQuery(query);
                }
            }
            else
            {
                ...
                for (int i = slvtpt; i < sltc; i++) //, '" + PSC + "'
                {
                    query = "insert into CT_PHIEUSUACHUA (MaPhieuSuaChua, MaTC) values (" + ma + " , " + TC.Rows[i][3].ToString() + ")";
                    re = DataProvider.Instance.ExecuteNonQuery(query);
                }
            }
        ...
        public void NhapVTPT(DataTable VTPT)//Thực hiện cập nhật lại số lượng vtpt còn lại trong kho.
        {
            ...
            {
                query = "Update VATTUPHUTUNG set SoLuong = SoLuong - " + VTPT.Rows[i][2] + "where MaVTPT = " + VTPT.Rows[i][5];
                int re = DataProvider.Instance.ExecuteNonQuery(query);
                i = i + 1;
            }
        }
        ...
        public bool KiemTraSoLuong(DataTable VTPTDangNhap, string MaVTPT, int SoLuong)//Kiểm tra số lượng nhập vào có nằm trong khả năng đáp ứng của kho.
        {
            ...
            int sl = int.Parse(dt.Rows[0][4].ToString());
            if (SoLuong <= sl)
            {
        ...
        public void CapNhatTienNo(string BienSo, int TienSuaChua)//Cập nhật tiền nợ của khách hàng có xe vừa được sửa chữa.
        {
            ...
            DataTable dt = DataProvider.Instance.ExecuteQuery("Select TienNo from XE where BienSo = '" + BienSo + "'");
            int TienNoCu = int.Parse(dt.Rows[0][0].ToString());
            int TienNoMoi = TienNoCu + TienSuaChua;
            int re = DataProvider.Instance.ExecuteNonQuery("Update XE set TienNo = " + TienNoMoi + " where BienSo = '" + BienSo + "'");
        }

        public DataTable HienThiThongTinPhieuSuachua(string BienSo)
        {
            string query = "Select XE.BienSo as 'Biển số', KHACHHANG.TenKH as 'Tên khách hàng', PHIEUSUACHUA.MaPhieusuaChua as 'Mã phiếu sửa chữa', TongTien as 'Tổng tiền' from XE, KHACHHANG, PHIEUSUACHUA, CT_PHIEUSUACHUA where XE.MaKH=KHACHHANG.MaKH and XE.BienSo=PHIEUSUACHUA.BienSo and PHIEUSUACHUA.MaPhieuSuaChua=CT_PHIEUSUACHUA.MaPhieuSuaChua and XE.BienSo = '" + BienSo + "' ";
