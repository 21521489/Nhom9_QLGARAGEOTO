...
namespace DAO
{
    public class BaoCaoTonDAO
    {
        ...
        public DataTable LayDtVTPT()
        {
            DataTable dt = DataProvider.Instance.ExecuteQuery("Select * from VATTUPHUTUNG");
            return dt;
        }
        ...
        {
            int amount = 0;
            DataTable dt = DataProvider.Instance.ExecuteQuery("Select * from CT_PHIEUNHAPVTPT ctpn, PHIEUNHAPVTPT pn where ctpn.MaPNVTPT = pn.MaPNVTPT AND MaVTPT = " + MaPhuTung + " and MONTH(ThoiDiem) = " + Thang + " and YEAR(ThoiDiem) = " + Nam);
            foreach (DataRow row in dt.Rows)
            {
        ...
        {
            int amount = 0;
            DataTable dt = DataProvider.Instance.ExecuteQuery("Select * from (XE join PHIEUSUACHUA on XE.BienSo = PHIEUSUACHUA.BienSo) join CT_PHIEUSUACHUA on PHIEUSUACHUA.MaPhieuSuaChua = CT_PHIEUSUACHUA.MaPhieuSuaChua where MaVTPT = " + MaPhuTung + " and MONTH(NgayTiepNhan) = " + Thang + " and YEAR(NgayTiepNhan) = " + Nam);
            foreach (DataRow row in dt.Rows)
            {
                amount += int.Parse(row["SoLuongVTPT"].ToString());
            }
            return amount;
        }
        ...
        public int LuongTonDauThang(int MaPhuTung, int Thang, int Nam)//Lấy lượng tồn đầu tháng của vtpt dựa vào lượng tồn cuối tháng trước
        {
            ...
            DataTable dt = DataProvider.Instance.ExecuteQuery("Select * from BAOCAOTON join CT_BAOCAOTON on BAOCAOTON.MaBCT = CT_BAOCAOTON.MaBCT where MaVTPT = " + MaPhuTung + " and MONTH(ThoiDiemBaoCao) = " + (Thang - 1) + " and YEAR(ThoiDiemBaoCao) = " + Nam);
            if (dt.Rows.Count > 0)
                foreach (DataRow row in dt.Rows)
                {
        ...
        {
            return DataProvider.Instance.ExecuteQuery("Select * from (BAOCAOTON join CT_BAOCAOTON on BAOCAOTON.MaBCT = CT_BAOCAOTON.MaBCT) join KHO on CT_BAOCAOTON.MaVTPT = KHO.MaPhuTung where MONTH(ThoiDiemBaoCao) = " + a.Month + " and YEAR(ThoiDiemBaoCao) = " + a.Year);
        }
